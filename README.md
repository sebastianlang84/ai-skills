...
- `parallel-agents` — Coordinating several agent sessions working the same repo or shared Brain at once — detecting live ownership, claiming Brain files, and stopping duplicate writes. Use when agents run in parallel, before editing shared Brain entries, when a merge turns up duplicate work, or when a Write is refused as a collision.
