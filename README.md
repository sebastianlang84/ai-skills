...
- `parallel-agents` — Coordinating several agent sessions working the same repo at once — detecting that another session is already on your topic, claiming work, and the PreToolUse hook that reports a file another worktree already has. Use when several sessions run in parallel, when a merge turns up duplicate work, or when deciding whether to start on something another agent may already hold.
