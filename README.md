...
- `pi-subagents` — Use when Pi subagent orchestration needs more than the subagent tool's built-in prompt guidance: role selection, multi-step handoffs, local agent-file/tool/model checks, CodeMap availability, or review discipline. Do not use for tiny/local tasks where the tool metadata is sufficient.
