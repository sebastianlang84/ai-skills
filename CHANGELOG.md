...
- Added `parallel-agents`: coordinating several sessions on one repo, plus the `PreToolUse` hook that enforces the part a written rule kept losing. Worktree isolation prevents interference, not duplication — two sessions creating the same new file is invisible to Git until merge, which happened here to two sessions each following the isolation rules perfectly. The hook reports when a file about to be created already exists in a sibling worktree, including **uncommitted** files in another live session, which no git command can see. Advisory by construction: it emits no permission decision, so nothing is blocked or auto-approved, and any error exits 0 — a hook that breaks `Write` is worse than no hook. Scoped to repos that actually have parallel checkouts, so it stays silent in ordinary single-worktree repos.
